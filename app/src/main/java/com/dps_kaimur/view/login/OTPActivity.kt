                ...
                try {
                    if(et_otp.text.toString().length==4){
                        ...
                        if(et_otp.text.toString() == et_otp.text.toString()){
                            ...
                            val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
                            imm.hideSoftInputFromWindow(view1.windowToken, 0)
                          //  Utils.showToast(this@OTPActivity, getString(R.string.action_sms), Color.WHITE)
                        }
                    }else{ }
                }catch (e:Exception){
                //    Utils.showToast(this@OTPActivity, getString(R.string.action_sms), Color.WHITE)
    ...
    private fun initJsonOperation(type:String,userIds:String,otp:String) {
        ...
        APIService!!.verifyOtp(requestBody).enqueue(object : Callback, retrofit2.Callback<ResponseModel> {
            override fun onResponse(call: Call<ResponseModel>?, response: Response<ResponseModel>?) {
                ...
                if (status != 200) {
                    when (status) {
                        ...
                        else -> Utils.showToast(this@OTPActivity,resources.getString(R.string.error_status_1), Color.WHITE)
                    }
                }
            }
            override fun onFailure(call: Call<ResponseModel>?, t: Throwable?) {
                ...
                Utils.showToast(this@OTPActivity, "Sorry!No internet available", Color.WHITE)
            }
        })
    }
